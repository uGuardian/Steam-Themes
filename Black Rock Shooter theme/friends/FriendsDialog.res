"Friends/FriendsDialog.res"
{
	...
	styles
	{
		...
		FriendsSearch
		{
			...
			{
				// background fill
				0="fill( x0 + 1, y0 + 1, x1+1, y1, groupertrans )"
		...
		FriendsSearchIcon
		{
			...
			{
				// background fill
				0="fill( x0 + 2, y0 - 1, x1 + 2, y1 +3, newgrouper1 )"
			}
		}
		...
		FriendsSearchIcon:disabled
		{
			...
			{
				// background fill
				0="fill( x0 + 1, y0 + 1, x1 - 1, y1 - 1, newgrouper1 )"
			}
		}		
 	...
 	layout
 	{
		...
		place { control="addFriendsButton" align=bottom margin-left=16 margin-right=9 margin-bottom=7 width=max height=24 }
 	}
